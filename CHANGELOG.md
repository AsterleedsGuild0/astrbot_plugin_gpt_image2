...
- `/image2 stats` 各站点余额表格新增“更新方式”列，“手动锚点估算”从余额值后拆分为独立“手动更新”标识。
- `/image2 stats` 各站点余额表格的“更新方式”优先按当前 Provider 费用配置判断，避免尚无缓存余额的站点显示为空。
- `/image2 stats` 各站点余额表格新增“单张开销”列，并按单张开销降序排列；余额缺少换算缓存时会按当前 Provider 费用配置补齐展示单位。
- 生图成功提示现在展示本次站点余额：自动余额观测显示 `，余额 12.1 CNY`；手动锚点估算显示 `，余额约 77.06 CNY（手动估算）`；无余额信息时不显示。
- 升级版本至 0.5.0。

### v0.5.0 Fixed

- Images API 多图请求遇到上游返回 `Unknown parameter: 'tools[0].n'` 时，现在会自动降级为多次单图请求，兼容内部转发到 Responses image tool 的 NewAPI-like 网关；同一站点触发一次后会持久化记忆，后续直接使用单图批量请求，避免重复探测不兼容参数。
...
- `/image2 stats` 的费用展示从“最近单次开销”调整为“今日、昨日、近 7 天、近 30 天”周期统计，并继续保留累计开销。
- `/image2 stats` 读取最近费用事件进行轻量聚合，不引入数据库依赖。
- 升级版本至 0.4.11。
...
- 生图与 Plan 流程的首次“已收到/处理中”提示优先使用 QQ/OneBot 表情反馈，非 QQ 或发送失败时回退原文字提示。
- 保留站点切换、配置错误、失败原因和最终结果等需要用户理解上下文的文字提示。
- 升级版本至 0.4.10。
...
- 费用观测默认余额单位和展示货币改为 `CNY`，并同步备用站费用示例。
- 生图成功提示中的费用来源改为中文展示，例如 `按余额差计算`。

### v0.4.9 Changed

- `/image2 stats` 拆分各站点统计宽表，并新增缓存余额、累计开销和最近开销展示。
- `/image2 balance` 开始查询时优先尝试 QQ/OneBot 表情反馈，失败时回退文字提示，并增加查询耗时日志。
- 升级版本至 0.4.9。
...
- 修复主站统计固定使用 `name:primary`，导致切换主站 `base_url` 后新主站继承旧主站统计的问题。

### v0.4.7 Changed

- 主站统计身份现在只绑定规范化后的 `base_url`，不受主站显示名、Images 模型或 Responses 模型变化影响。
- Provider stats schema 升级到 v5，迁移时会删除不再受支持的旧 `name:primary` 主站统计，避免污染数据继续影响当前主站或历史统计展示。
- 升级版本至 0.4.7。
